...
// WebSocket chat/signaling channel letiables.
...
function sendToServer(msg) {
  ...
  printLog("Sending '" + msg.type + "' message: " + msgJSON);
  connection.send(msgJSON);
}

// Called when the "id" message is received; this message is sent by the
// server to assign this printLogin session a unique ID number; in response,
...
function connect() {
  ...
  connection.onmessage((evt) => {
    ...
    switch (msg.type) {
      ...
      default:
        printLogError("Unknown message received:");
        printLogError(msg);
    }
...
async function createPeerConnection() {
  printLog("Setting up a connection...");
...
async function handleNegotiationNeededEvent() {
  printLog("*** Negotiation needed");

  try {
    printLog("---> Creating offer");
    ...
    if (myPeerConnection.signalingState != "stable") {
      printLog("     -- The connection isn't stable yet; postponing...");
      return;
    }
    ...
    printLog("---> Setting local description to the offer");
    await myPeerConnection.setLocalDescription(offer);

    // Send the offer to the remote peer.

    printLog("---> Sending the offer to the remote peer");
    sendToServer({
      name: myUsername,
...
function handleICECandidateEvent(event) {
  if (event.candidate) {
    printLog("*** Outgoing ICE candidate: " + event.candidate.candidate);
...
//
// We don't need to do anything when this happens, but we printLog it to the
...
// Close the RTCPeerConnection and reset letiables so that the user can
...
function closeVideoCall() {
  ...
  printLog("Closing the call");

  // Close the RTCPeerConnection

  if (myPeerConnection) {
    printLog("--> Closing the peer connection");
...
function handleHangUpMsg(msg) {
  printLog("*** Received hang up notification from other peer");
...
async function invite(evt) {
  printLog("Starting to prepare an invitation");
  if (myPeerConnection) {
    ...
    printLog("Inviting user " + targetUsername);
    ...
    printLog("Setting up connection to invite user: " + targetUsername);
...
async function handleVideoOfferMsg(msg) {
  ...
  printLog("Received video chat offer from " + targetUsername);
  if (!myPeerConnection) {
    createPeerConnection();
  }
  ...
  if (myPeerConnection.signalingState != "stable") {
    ...
    printLog("  - Setting remote description");
  ...
  printLog("---> Creating and sending answer to caller");
...
async function handleVideoAnswerMsg(msg) {
  printLog("*** Call recipient has accepted our call");
...
async function handleNewICECandidateMsg(msg) {
  ...
  printLog("*** Adding received ICE candidate: " + JSON.stringify(candidate));
  try {
...
function reportError(errMessage) {
  printLogError(`Error ${errMessage.name}: ${errMessage.message}`);
}
